...
import { Save, Loader2, ArrowLeft, Wand2, Sparkles } from 'lucide-react'; // 🟢 Added Magic Icons
import Link from 'next/link';

export default function AddProduct() {
  const [loading, setLoading] = useState(false);
  const [generating, setGenerating] = useState(false); // 🟢 For the AI Effect
  ...
  // 🤖 THE "AI" ENGINE (Simulated Intelligence)
  const generateMagicDescription = () => {
    ...
    // 1. "Thinking" Delay (To feel like AI)
    setTimeout(() => {
        let aiText = "";
        ...
        switch(category) {
            case "Food":
                aiText = `Experience the authentic taste of ${name}. Freshly sourced and prepared with the finest local ingredients. Perfect for family meals or a healthy treat. Taste the difference of premium quality.`;
                break;
            case "Fashion":
                aiText = `Step out in style with this stunning ${name}. Crafted for elegance and comfort, this piece is perfect for special occasions or elevating your everyday look. Available now for the fashion-forward individual.`;
                break;
            case "Beauty":
                aiText = `Enhance your natural glow with our ${name}. Dermatologically tested and safe for all skin types. This premium formula helps you look and feel your absolute best. A must-have for your routine.`;
                break;
            case "Electronics":
                aiText = `Upgrade your tech game with the ${name}. Features state-of-the-art performance, durability, and a sleek design. Tested for quality and reliability.`;
                break;
            default:
                aiText = `Discover the excellence of ${name}. High-quality, durable, and designed to meet your needs. One of our top-rated items in the store.`;
        }

        setDescription(aiText);
        setGenerating(false);
    }, 1500); // 1.5 second "Brainstorming" delay
  };
