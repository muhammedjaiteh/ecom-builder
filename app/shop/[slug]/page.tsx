      ...
      return (
        <div className="mt-8 grid grid-cols-2 gap-x-4 gap-y-12 md:grid-cols-3 lg:grid-cols-4">
          {products.map((product) => {
            ...
            return (
              <article key={product.id} className="group flex flex-col">
                <Link href={`/product/${product.id}`} className="block overflow-hidden bg-white border border-neutral-100">
                  <div className="relative w-full aspect-[4/5]">
                    <img src={imageUrl} alt={product.name} className="h-full w-full object-cover transition-opacity duration-500 group-hover:opacity-90" />
                  </div>
                </Link>
                <div className="mt-3 flex flex-col items-center text-center">
                  <Link href={`/product/${product.id}`}>
                    <h3 className="text-[11px] uppercase tracking-widest text-neutral-500 mb-1 truncate w-full px-2">{product.name}</h3>
                  </Link>
                  <p className="text-sm font-medium text-neutral-900">D{product.price}</p>
                </div>
              </article>
            );
          })}
