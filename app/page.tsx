...
type Product = {
  ...
  shop?: { shop_name: string; shop_slug: string }; // Attached for search results
};
...
export default function GlobalHomepage() {
  ...
  useEffect(() => {
    async function fetchCuratedMall() {
      const { data, error } = await supabase
        ...
        `)
        .limit(20); // Increased limit to fetch more of the mall
  ...
  // UPGRADE 2: The Global Search Engine
    ...
    shops.forEach(shop => {
      shop.products.forEach(product => {
        if (
          ...
          // Attach shop info so buyers know who is selling it
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans text-gray-900 selection:bg-gray-900 selection:text-white">
      ...
      {/* UPGRADE 3: THE GLOBAL SEARCH BAR */}
      <section className="sticky top-[73px] z-40 border-b border-gray-200 bg-white/95 backdrop-blur-md px-4 py-4 md:px-10">
        <div className="mx-auto max-w-3xl relative">
          <div className="flex items-center w-full overflow-hidden rounded-full border-2 border-gray-200 bg-gray-50 focus-within:border-gray-900 focus-within:bg-white transition-all">
            ...
            <input
              ...
              placeholder="Search across all boutiques (e.g. Nike, Honey, Dress)..."
              className="w-full bg-transparent py-4 text-sm font-medium text-gray-900 outline-none placeholder:text-gray-400"
            />
            {searchQuery && (
              <button 
                onClick={() => { setSearchQuery(''); setIsSearching(false); }}
                className="px-5 text-gray-400 hover:text-gray-900 transition-colors"
              >
                <X size={20} />
              </button>
            )}
      ...
      {/* 3. CURATED WORLDS (Hides when searching) */}
      {!isSearching && (
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 md:px-10">
        ...
        {/* UPGRADE 4: SEARCH RESULTS VIEW */}
        {isSearching ? (
          <div>
            <h2 className="mb-8 text-2xl font-serif text-gray-900">
              Search Results for <span className="font-bold text-gray-500">"{searchQuery}"</span>
            </h2>

            {searchResults.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-white py-24 text-center px-4">
                <Search className="h-12 w-12 text-gray-300 mb-4" />
                <h3 className="text-xl font-serif text-gray-900">No items found</h3>
                <p className="mt-2 text-sm text-gray-500">Try searching for a different product or category.</p>
              </div>
            ) : (
                ...
                {searchResults.map((product) => {
                  ...
                  return (
                    <Link href={`/product/${product.id}`} key={product.id} className="group flex flex-col">
                      <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-gray-100 border border-gray-200">
                        {imgUrl ? (
                        ...
                        <div className="absolute top-2 left-2 rounded-md bg-white/90 backdrop-blur px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-gray-900 shadow-sm">
                          {product.shop?.shop_name}
                        </div>
                      </div>
                      <div className="mt-3">
                        <h4 className="truncate text-sm font-bold text-gray-900 group-hover:underline">{product.name}</h4>
                        <p className="text-sm font-medium text-gray-500">D{product.price}</p>
                      </div>
                    </Link>
                  );
                })}
          ...
          /* NORMAL BOUTIQUE VIEW (Shows when not searching) */
          <div>
            ...
            {displayedShops.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-white py-24 text-center px-4 shadow-sm">
                ...
                {shops.length === 0 && (
                  <Link href="/register" className="mt-8 rounded-full bg-gray-900 px-8 py-3 text-sm font-bold uppercase tracking-widest text-white hover:bg-black transition-colors">
